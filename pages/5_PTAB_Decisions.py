...
if selected_rows:
    raw = decisions[selected_rows[0]]
    tm  = raw.get("trialMetaData", {})
    po  = raw.get("patentOwnerData", {})
    rp  = raw.get("regularPetitionerData", {})
    trial_num = raw.get("trialNumber", "")

    st.markdown("---")
    st.subheader("Decision Detail: " + trial_num)

    c1, c2, c3 = st.columns(3)
    c1.metric("Type",     tm.get("trialTypeCode", "N/A"))
    c2.metric("Status",   tm.get("trialStatusCategory", "N/A"))
    c3.metric("Patent #", po.get("patentNumber", "N/A"))

    c4, c5, c6 = st.columns(3)
    c4.metric("Filed",       tm.get("accordedFilingDate", "N/A"))
    c5.metric("Institution", tm.get("institutionDecisionDate", "N/A"))
    c6.metric("FWD Date",    tm.get("finalWrittenDecisionDate", "N/A"))

    st.markdown("**Patent Owner:** " + po.get("realPartyInInterestName", "N/A"))
    st.markdown("**Petitioner:** " + rp.get("realPartyInInterestName", "N/A"))
    st.link_button("View Full Decision on PTAB Portal",
                   "https://ptab.uspto.gov/#/search/" + trial_num)
...
if OPENAI_API_KEY:
    ...
    if trial_num:
        if st.button("Analyse with AI"):
            st.info(
                "Full decision text for " + trial_num + " is not yet available via the USPTO API. "
                "Download the decision PDF from the PTAB Portal link above, then paste the text here."
            )
            decision_text = st.text_area("Paste decision text:", height=250)
            if decision_text and st.button("Run Analysis", key="run_analysis"):
        ...
        st.caption("Select a decision row above to enable AI analysis.")
