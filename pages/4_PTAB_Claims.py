    ...
    if not openai_service._check():
        ...
        if not claims_text:
            ...
            if analyse and cache_key_claims not in st.session_state:
                ...
                st.session_state[cache_key_claims] = analysis

            if cache_key_claims in st.session_state:
                analysis = st.session_state[cache_key_claims]
                st.markdown("---")
                st.markdown("### Claim Analysis")
                st.markdown(analysis)

                st.download_button(
                    "⬇️ Export Claim Analysis",
                    data=f"# Pre-filing Claim Analysis\n\n## Claims\n\n{claims_text}\n\n## Analysis\n\n{analysis}",
