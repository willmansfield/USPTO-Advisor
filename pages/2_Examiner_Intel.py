...
def _score_badge(score, band, color):
    return (
        ...
        f'border-radius:24px;font-weight:700;font-size:1.2rem;">'
...
with tab_ex:
    ...
    # Clickable rows — select → open in Prosecution Hub
    event = st.dataframe(
        df,
...
with tab_au:
    ...
    st.markdown(_score_badge(au_stats["score"], au_stats["band"], au_stats["color_hex"]),
                unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
